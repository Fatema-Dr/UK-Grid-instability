...
Validation against the August 9, 2019 UK blackout demonstrates the system's effectiveness: **7-second advance warning** was achieved before frequency collapsed to 48.8 Hz. Model performance metrics include Pinball Loss of 0.0142 (lower bound), MAE of 0.033 Hz, and real-time dashboard latency of 0.40 seconds. Feature importance analysis confirms physics alignment: RoCoF contributed 38.2% of predictive power, validating the domain-informed approach.

The research demonstrates that proactive early warning is achievable for low-inertia grids, though seasonal generalisability requires cross-season training. GridGuardian represents proof-of-concept for transitioning grid stability management from reactive response to predictive prevention.
...
| **PICP (%)** | 77.5–81.5 | — | ≥80% | Marginal |
| **MPIW (Hz)** | 0.12 | 0.15 | <0.2 | Pass |
| **Calibration (Observed < α=0.1)** | 1.8% | — | 10% | Pessimistic |
...
Table 4.2 presents comprehensive December 2019 validation results, testing model generalisability beyond training conditions. This represents a **stress test** of the model's robustness.
...
|--------|------------------|-------------------|--------|------------|
| **PICP (%)** | 79.5 | 74.2 | -5.3 pp | Below target |
| **Pinball Loss (Lower)** | 0.0142 | 0.0161 | +13.4% | Degraded |
| **Pinball Loss (Upper)** | 0.0168 | 0.0189 | +12.5% | Degraded |
| **MAE Lower (Hz)** | 0.033 | 0.041 | +24.2% | Significant degradation |
| **MAE Upper (Hz)** | 0.018 | 0.023 | +27.8% | Significant degradation |
| **RMSE Lower (Hz)** | 0.047 | 0.059 | +25.5% | Significant degradation |
| **RMSE Upper (Hz)** | 0.029 | 0.038 | +31.0% | Significant degradation |
| **Calibration (α=0.1)** | 1.8% | 0.9% | -0.9 pp | More pessimistic |
| **Calibration (α=0.9)** | 79.3% | 75.6% | -3.7 pp | Under-confident |
| **MPIW (Hz)** | 0.135 | 0.168 | +24.4% | Wider intervals |
| **F1-Score** | 0.90 | 0.84 | -6.7% | Reduced accuracy |
| **False Negative Rate** | 0.12 | 0.18 | +6.0 pp (+50% relative) | Concerning increase |


### Analysis of Performance Degradation

The comprehensive metrics reveal **severe seasonal overfitting** with concerning implications:

**Coverage Failure (PICP -5.3 pp)** The Prediction Interval Coverage Probability dropped from 79.5% to 74.2%, falling significantly below the 80% target. This means the model's uncertainty bands are **too narrow** for winter conditions—predictions are over-confident, potentially missing true frequency variations.

**Accuracy Degradation (MAE +24-28%)** Mean Absolute Error increased by approximately 25% across both quantiles. For a safety-critical system targeting <0.05 Hz accuracy, this degradation pushes error margins uncomfortably close to operational thresholds.

**False Negative Surge (+6 pp, 50% relative increase)** Most concerning is the increase in False Negative Rate from 12% to 18%—a 6 percentage point absolute increase representing a 50% relative increase. False negatives represent **missed instability events**—exactly the failures the system aims to prevent. This degradation suggests the model is less reliable at detecting winter instability patterns.

**Prediction Interval Widening (MPIW +24%)** While the model becomes less accurate, it paradoxically produces wider prediction intervals. This indicates **poor uncertainty quantification**: the model is both less precise *and* less appropriately uncertain about its predictions.

### Root Cause Analysis

Four factors explain the winter degradation:

**1. Out-of-Distribution Solar Feature** Solar radiation contributed 2.5% feature importance in August but is functionally zero in December (negligible solar generation). The model learned weights for this feature that became irrelevant, effectively reducing the effective feature space.

**2. Temperature-Demand Coupling** In August, temperature showed weak correlation with frequency (0.3% importance). In December, cold snaps trigger heating demand spikes that strain generation reserves—a correlation the August-trained model never learned.

**3. Different Wind Patterns** Winter wind exhibits different statistical properties: higher average speeds, different diurnal patterns, and correlation with cold fronts that affect thermal generation availability.

**4. Inertia Pattern Changes** The daily inertia cost proxy assumes consistent patterns across seasons. In reality, winter evenings exhibit more severe inertia reductions due to heating-driven demand peaks coinciding with low solar output.

### Implications for Generalisability

These results demonstrate that the current model is **not merely season-specific but potentially dangerous** if deployed without retraining. The 50% false negative increase indicates systematic blind spots for winter conditions.

**Recommendation:** The model requires **mandatory retraining** for each season or (preferably) training on multi-season data spanning at least 12 months. Claims of generalisability are **not supported** by the evidence. See Chapter 6, Section 6.4.4 for detailed recommendations on cross-season training.
...
*SIL = Safety Integrity Level. Gap severity: Critical (blocking), Moderate (addressable).*## 5.6 Theoretical Contributions
...
This research demonstrated that proactive early warning of power grid instability is achievable through physics-informed machine learning. The GridGuardian system successfully predicted the August 9, 2019 UK blackout 7 seconds before actual frequency collapse, providing sufficient time for automated response systems to activate.
...
| **PICP (%)** | 77.5–81.5 | — | ≥80% | Marginal | Slightly below nominal target |
| **MPIW (Hz)** | 0.12 | 0.15 | <0.2 | Pass | Reasonable precision-uncertainty balance |
| **Calibration (Observed < α=0.1)** | 1.8% | — | 10% | Pessimistic | Desirable for safety-critical applications |
| **Calibration (Observed < α=0.9)** | 79.3% | — | 90% | Slightly Low | Upper bound coverage acceptable |
...
| **MAE (Hz)** | 0.033 | 0.041 | +24.2% | Degradation |
| **Calibration (α=0.1)** | 1.8% | 0.9% | -0.9 pp | Increased pessimism |
| **Calibration (α=0.9)** | 79.3% | 75.6% | -3.7 pp | Below target |
| **F1-Score** | 0.90 | 0.84 | -6.7% | Reduced accuracy |
...
|------------|-------|------------------|----------------------|---------------------|--------|
| 16:52:30 | Pre-disturbance (nominal) | 50.00 | 49.95 | — | Stable |
| 16:52:33 | **Alert threshold crossed** | 49.91 | **49.80** | **7** | Warning |
| 16:52:35 | Frequency declining | 49.85 | 49.76 | 5 | Critical |
...
|----------|----------|----------|-----------|--------|
| **α=0.1** | 10.0% | 1.8% | -8.2 pp | Pessimistic |
| **α=0.9** | 90.0% | 79.3% | -10.7 pp | Slightly Low |
